...
import type { WinState, StickyNote, IDesktopContext } from '../types/desktop';
import { WhoYouPlay } from '../features/WhoYouPlay';
...
export class WinXPDesktop extends Scene {
  ...
  wins = new Map<string, WinState>();
  private explorers = new Map<string, FileExplorer>();
  tbBtns = new Map<string, HTMLElement>();
  ...
  private wyp!: WhoYouPlay;

  constructor() { super('WinXPDesktop'); }

  /** Returns the next z-index value. Satisfies IDesktopContext. */
  nextZ(): number { return ++this.zTop; }
  ...
  private buildDesktopIcons() {
    ...
    area.appendChild(this.makeDesktopIconImg('Who You Play?', '/wyp-icon.svg', () => this.wyp.open()));
  }
  ...
  trackRecent(id: string, label: string, iconHtml: string, action: () => void) {
